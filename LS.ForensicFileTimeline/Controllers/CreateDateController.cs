...
{
    public class CreateDateController : Controller
    {
        ...
        public CreateDateController(IRecordService userService, IMapper mapper, ITimelineService service, IFileService fileService)
        {
            _service = service;
            _mapper = mapper;
        ...
        public ActionResult Record_Read([DataSourceRequest] DataSourceRequest request, string date, bool img, bool vid, bool other, string path)
        {
            if (img || vid || other)
            {
                ...
                var files = _fileService.GetAll().Where(x => x.Created.Date == filterDate);
        ...
        public JsonResult GetFiles(bool img, bool vid, bool other, bool nodate)
        {
            if (img || vid || other || nodate) {
                var files = _fileService.GetAll().Where(x => x.Created != DateTime.Now.Date);
                ...
                foreach (var items in newList)
                {
                    var result = new TimelineModel
                    {
                        ...
                        Title = "created " + items[0].Created.Date.ToString("MM/dd/yyyy"),
        ...
        public void ViewBags(string date)
        {
            ...
            if (!string.IsNullOrEmpty(date))
                files = _fileService.GetAll().Where(x => x.Created.Date == Convert.ToDateTime(date).Date).ToList();
            ...
            var noDateNr = files.Where(x => x.Created == DateTime.Now.Date).Count();
