...
namespace LS.ForensicFileTimeline.Controllers
{
    public class ModifiedController : Controller
    {
        ...
        public ModifiedController(IRecordService userService, IMapper mapper, ITimelineService service, IFileService fileService)
        {
            _service = service;
            _mapper = mapper;
        ...
        public ActionResult Record_Read([DataSourceRequest] DataSourceRequest request, string date, bool img, bool vid, bool other, bool nodate, string path)
        {
            if (img || vid || other)
            {
                var filterDate = date != null ? Convert.ToDateTime(date).Date : DateTime.Now.Date;
                var files = _fileService.GetAll().Where(x => x.Modified.Date == filterDate);

                if (nodate)
                    files = _fileService.GetAll().Where(x => x.Modified.Date == filterDate);
        ...
        public JsonResult GetFiles(bool img, bool vid, bool other, bool nodate)
        {
            ...
            {
                var files = _fileService.GetAll().Where(x => x.Modified != DateTime.Now.Date);
                ...
                var newList = files.GroupBy(p => p.Modified.Date).Select(p => p.ToList()).ToList();
                ...
                foreach (var items in newList)
                {
                    ...
                    {
                        Date = items[0].Modified.Date,
                        Title = items[0].Modified.Date.ToString("MM/dd/yyyy"),
                        Description = $"{items.Count} files",
                        Actions = new List<TimelineEventModelAction>() {
                                new TimelineEventModelAction() {
                                    text = "More details",
                                    url="/grid/index?date="+ items[0].Modified.Date.ToString("MM/dd/yyyy")}
                              }
                    };
        ...
        public void ViewBags(string date)
        {
            ...
            if (!string.IsNullOrEmpty(date))
                files = _fileService.GetAll().Where(x => x.Modified.Date == Convert.ToDateTime(date).Date).ToList();
            ...
            var noDateNr = files.Where(x => x.Modified == DateTime.Now.Date).Count();
